...
const gunhee : Retriever = new Retriever('Gunhee');
...
type T2 = keyof typeof constCart;
type T3 = 1 | 2 | 3;
...
try {
    ...
    // 내 코드
    const message = error instanceof Error ? error.message : (Array.isArray(error) ? error.join(' ') : error);
    console.log(message); // (라)

    // 정답
    console.log(toErrorWithMessage(error)); // (마)
}
...
function deleteArray(arr: TUser[] | number[], startOrKey: TPropertyKeyType, endOrValue?: unknown) {
    ...
    //     const arr2 : TUser[] = [];
    //     arr.filter((e) => {
    //         if (e && typeof e === 'object' && e[startOrKey] != endOrValue) {  
    //             arr2.push(e);          
    //         }
    //     });
    //     return arr2;
